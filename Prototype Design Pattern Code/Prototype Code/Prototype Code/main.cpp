...
public:
	...
	Rectangle(Rectangle* t_source)
	{
		//Rectangle* thing = t_source;

		Shape(t_source);
		this->width = t_source->width;
		this->height = t_source->height;
	};
...
public:
	...
	Circle(Circle* t_source)
	{
		Shape(*t_source);
		this->radius = t_source->radius;
	};
...
public:
	Application()
	{
		...
		circle.radius = 20;
		m_shapesArray.push_back(&circle);

		Circle anotherCircle = *circle.clone();
		m_shapesArray.push_back(&anotherCircle);

		Rectangle rectangle = new Rectangle();
		rectangle.width = 10;
		rectangle.height = 10;

		m_shapesArray.push_back(&rectangle);
	...
	void businessLogic()
	{
		std::vector<Shape*> copyShapesArray;

		for (Shape* s: m_shapesArray)
		{
			copyShapesArray.push_back(s->clone());
		}
	};

private:

	std::vector<Shape*>m_shapesArray;
};
